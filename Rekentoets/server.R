...
shinyServer(function(input, output, session) {
  ...
  n <- sample(1:length(questionsMC$txt), size =10)
  ...
  ##Build user interface with 10 MC. Save choices
  ...
  output$ui2 <- renderUI({fluidPage(withMathJax(
    ...
    radioButtons('answ6', paste('V6:',vraagMC$txt[6]),
                 c(vraagMC[6,g[1]],
                   vraagMC[6,g[2]],
                   vraagMC[6,g[3]],
                   vraagMC[6,g[4]]), selected = 0),
    
    radioButtons('answ7', paste('V7:',vraagMC$txt[7]),
                 c(vraagMC[7,h[1]],
                   vraagMC[7,h[2]],
                   vraagMC[7,h[3]],
                   vraagMC[7,h[4]]), selected = 0),
    
    radioButtons('answ8', paste('V8:',vraagMC$txt[8]),
                 c(vraagMC[8,i[1]],
                   vraagMC[8,i[2]],
                   vraagMC[8,i[3]],
                   vraagMC[8,i[4]]), selected = 0),
  ...
  rltInput4 <- reactive({input$goButton 
                         isolate(try_default(chkQuestion(input$answ4, values$corr4),
                                             default = 'Leeg', quiet = TRUE))
                         })
                         
  rltInput5 <- reactive({input$goButton 
                         isolate(try_default(chkQuestion(input$answ5, values$corr5),
                                            default = 'Leeg', quiet = TRUE))
                         }) 
                         
  rltInput6 <- reactive({input$goButton 
                         isolate(try_default(chkQuestion(input$answ6, values$corr6),
                                            default = 'Leeg', quiet = TRUE))
                         })
  ...
  output$scr <- renderText({paste0('Je score is: ',scrInput1() + scrInput2() + scrInput3() + scrInput4() + scrInput5()  + scrInput6() + scrInput7() + scrInput8() + scrInput9() + scrInput10(),
                                   ' (of ', round((scrInput1() + scrInput2() + scrInput3() + scrInput4() + scrInput5() + scrInput6()  + scrInput7() + scrInput8() + scrInput9() + scrInput10())*100/20,0),
                                   '%)')})
  ...
  observe({if(length(trials()) != 0) {
          quiz <- data.frame(Sys.time(), urlText(), session$clientData$url_search, 
                             ...
                             scrInput1(), scrInput2(),scrInput3(), scrInput4(), scrInput5(), scrInput6(), scrInput7(), scrInput8(), scrInput9(), scrInput10())
